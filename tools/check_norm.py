...
browser voice while everything around it is fine. Nothing is logged. On iOS with
no synthesiser voice loaded, that line is simply silent.

Runs the whole corpus plus every manifest key through both implementations and
diffs them. Seconds to run, and the only thing that catches the failure.
...
# Loading audio.js in node needs stubs: it ends with a bare AudioLib.init() that
# calls fetch(), document.addEventListener and document.querySelector at load
# time, derives AUDIO_BASE from location, and its `const AudioLib` does not
# escape eval() on its own.
JS = r'''
...
// argv[2], not argv[1]: this runs a real script file, so the arg shifts by one.
...
def main():
    ...
    strings = list(prose) + list(numbers)
    mpath = os.path.join(ROOT, 'audio', 'manifest.json')
    if os.path.exists(mpath):
        with open(mpath, encoding='utf-8') as f:
            strings += list(json.load(f)['words'])
    ...
    r = subprocess.run(['node', js_file, src], cwd=ROOT, capture_output=True, text=True)
    if r.returncode != 0:
        print('node failed:\n' + r.stderr[:800])
        return 1
