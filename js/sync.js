...
//
// Adapted from wonder-lab/js/sync.js. Two things change per app and nothing
// else: the document name in doc() and the localStorage device-id key.
//
// Storage: profiles/{uid}/apps/math-lab
//   { name, progress, updatedAt: serverTimestamp, deviceId }
// Each app owns its own document, so two apps can never clobber each other
// while still sharing the login. The credentials are derived in
// js/firebase-config.js (HOMESCHOOL_AUTH) precisely so that derivation cannot
// drift apart between apps.
//
// This app writes a SECOND, much smaller document — the family summary that the
// parent dashboard reads. See js/family.js for why it exists and what is in it:
// the deployed rule on profiles/{uid} is `request.auth.uid == uid`, so a parent
// signed in as themselves cannot read a child's progress document at all.
//
...
const Sync = {
  ...
  deviceId() {
    let id = localStorage.getItem('mathlab:deviceId');
    if (!id) {
      id = 'd' + Math.random().toString(36).slice(2, 12);
      localStorage.setItem('mathlab:deviceId', id);
    }
    return id;
  },
  ...
  async push(profile) {
    ...
    try {
      ...
      // The parent dashboard's copy. Small, derived, and written in the same
      // pass so it can never lag the real record by more than one save.
      if (typeof Family !== 'undefined') {
        await Family.publish(this, profile).catch(() => {});
      }
    } catch (e) {
  ...
  // through this listener too, which is what deviceId filters out — without it
  // the app would reload its own state on every save.
  watch(onRemote) {
...
// window — so every `window.Sync && ...` guard elsewhere silently evaluates
// false and cloud sync becomes dead code that looks fine. Publish it.
window.Sync = Sync;
